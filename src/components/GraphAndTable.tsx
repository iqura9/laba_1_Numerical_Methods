...
export const g = (x: number): number =>
  (x ** 2 * -1 + 3 - 2 * Math.sin(x - 4)) / 5;
export const f = (x: number): number =>
  2 * Math.sin(x - 4) + x ** 2 + 5 * x - 3;
export const fDerivative = (x: number): number =>
  2 * Math.cos(x - 4) + 2 * x + 5;
...
const GraphAndTable: FC<IGraphAndTable> = memo(
  ({ epsilon, func, name, w, children }) => {
